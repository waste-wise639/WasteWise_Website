...
export default function Pricing() {
  return (
    <section
      id="pricing"
      className="relative py-20 lg:py-28 px-6 md:px-10 lg:px-[58px] isolate"
      style={{ background: "linear-gradient(180deg, #FFFFFF 0%, #F5FAF5 50%, #FFFFFF 100%)" }}
    >
      <div
        className="absolute left-1/2 -translate-x-1/2 -top-4 w-[600px] h-[600px] rounded-full pointer-events-none z-0"
        style={{ background: "rgba(9,179,9,0.1)", filter: "blur(32px)" }}
      />

      <div className="relative z-10 max-w-[1280px] mx-auto flex flex-col items-center gap-14 px-0 lg:px-8">
        <motion.div
        ...
        >
          ...
          <h2 className="text-[32px] lg:text-[48px] font-extrabold leading-[1] text-center text-[#171C1A] font-[family-name:var(--font-jakarta)]">
            Simple, Transparent <span className="text-[#09B309]">Pricing</span>
          </h2>
          <p className="text-lg leading-7 text-center text-[#6D7873]">
            Pay only for the service you need. Whether you&apos;re a household or a waste professional.
          </p>
        </motion.div>

        <div className="flex flex-col lg:flex-row justify-center gap-8 max-w-[896px] w-full">
          {/* Household Card */}
          <motion.div
            className="flex-1 flex flex-col p-10 gap-2 bg-white border border-[#E3E8E3] rounded-3xl shadow-[0px_10px_15px_-3px_rgba(0,0,0,0.1),0px_4px_6px_-4px_rgba(0,0,0,0.1)]"
          ...
          {/* Vendor Card */}
          <motion.div
            className="flex-1 flex flex-col p-10 gap-2 bg-[#171C1A] border border-[#171C1A] rounded-3xl shadow-[0px_25px_50px_-12px_rgba(0,0,0,0.25)]"
            initial={{ opacity: 0, y: 30 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
